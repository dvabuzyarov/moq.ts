import { createInjector, resolve2, resolveMock } from "../../tests.components/resolve.builder";
import { StaticInjector } from "../static.injector/injector";
import { It, Mock } from "moq.ts";
import { ObjectFormatter } from "./object.formatter";
import { IteratorFormatter } from "./iterator.formatter";
import { IteratorTester } from "./iterator.tester";

describe("Iterator formatter", () => {

    beforeEach(() => {
        createInjector(IteratorFormatter, [StaticInjector, IteratorTester]);
    });

    beforeEach(() => {
        resolveMock(StaticInjector).prototypeof(StaticInjector.prototype);
    });

    it("Returns formatted value", () => {
        const number = 1;
        const string = "string value";
        const value = [number, string];

        const objectFormatter = new Mock<ObjectFormatter>()
            .setup(instance => instance.format(number))
            .returns("1")
            .setup(instance => instance.format(string))
            .returns("2")
            .object();
        resolveMock(StaticInjector)
            .setup(instance => instance.get(ObjectFormatter))
            .returns(objectFormatter);
        resolveMock(IteratorTester)
            .setup(instance => instance.verify(value))
            .returns(true);

        const matcher = resolve2(IteratorFormatter);
        const actual = matcher.format(value);

        expect(actual).toBe("Iterable([1,2])");
    });

    it("Returns undefined for unsupported types", () => {

        resolveMock(IteratorTester)
            .setup(instance => instance.verify(It.IsAny()))
            .returns(false);

        const provider = resolve2(IteratorFormatter);
        const actual = provider.format(undefined);

        expect(actual).toBe(undefined);
    });
});
